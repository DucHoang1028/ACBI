...
- `GET /api/conversations`: returns the current user's visible saved conversations. `GET /api/conversations/{id}` returns the whole conversation as a `transcript` (every saved answer in order, with any clarification turns still remembered between them) plus a `results` id list; the UI uses it to reopen a conversation with all its turns. `GET /api/results/{id}` reopens the stored answer and original provenance. Ownership and current role/factory scope are checked again on every read.
