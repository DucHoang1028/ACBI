...
const FAQS: Faq[] = [
  ...
  {q: t('Hệ thống không làm được gì?', 'What can it not do?'), a: t('Không giải thích nguyên nhân ("tại sao"), không chia theo quý, không so sánh hai kỳ tùy ý, không trả lời về nhân sự, lương, khách hàng hay lợi nhuận, và không sửa dữ liệu (chỉ đọc).', 'It cannot explain reasons ("why"), split by quarter, compare two arbitrary periods, or answer about staff, salaries, customers or profit, and it never changes data (read-only).')},
];
