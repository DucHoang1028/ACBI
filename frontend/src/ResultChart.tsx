...
const names:Record<string,[string,string]>={
  ...
  defect_rate:['Tỷ lệ phế phẩm','Defect rate'], on_time_rate:['Tỷ lệ đúng hạn','On-time rate'], sales_growth:['Tăng trưởng doanh thu','Sales growth'],
  current_revenue:['Doanh thu kỳ này','Current revenue'],previous_revenue:['Doanh thu kỳ trước','Previous revenue'],
  actual:['Thực tế','Recorded'],forecast:['Dự báo','Forecast'],
};
...
export function ResultChart({config,rows,language}:{config:VizConfig|null;rows:Record<string,unknown>[];language:'vi'|'en'}) {
  ...
  const format=(value:number,key:string)=>new Intl.NumberFormat(language==='vi'?'vi-VN':'en-US',
    {style:['defect_rate','sales_growth','on_time_rate'].includes(key)?'percent':'decimal',maximumFractionDigits:2}).format(value);
  ...
  let data=rows.map(row=>{
    const value={...row};
    for(const key of config.y)value[key]=row[key]===null||row[key]===undefined?null:Number(row[key]);
  ...
  return <figure className="result-chart" aria-label={label(metric)}><figcaption>{label(metric)}</figcaption>
    <ResponsiveContainer width="100%" height={320}>
      ...
      :config.type==='line'?<LineChart data={data}>{axes}{series.map((s,i)=><Line key={s.key} type="linear" dataKey={s.key} name={s.name} stroke={colors[i%colors.length]} strokeWidth={2} strokeDasharray={s.key==='forecast'?'6 4':undefined} connectNulls={false} isAnimationActive={false}/>)}</LineChart>
