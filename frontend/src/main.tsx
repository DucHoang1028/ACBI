...
function App() {
  ...
  const [demoUsers,setDemoUsers]=useState<{username:string;role:string}[]>([]);
  ...
  useEffect(()=>{
    ...
    fetch('/api/auth/demo-users').then(r=>r.ok?r.json():[]).then(setDemoUsers).catch(()=>{});
  },[]);
  useEffect(()=>{
  ...
  async function demoSignIn(name:string){
    setBusy(true);setError('');
    try{
      const r=await fetch('/api/auth/demo-login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:name})});
      if(!r.ok)throw Error();setAccess((await r.json()).access_token);
    }catch{setError(vi?'Không đăng nhập nhanh được.':'Quick sign-in failed.');}
    finally{setBusy(false);}
  }
  async function signOut(){await fetch('/api/auth/logout',{method:'POST'}).catch(()=>{});setAccess('');setPassword('');}
  function openGuide(on:boolean){setGuide(on);history.replaceState(null,'',on?'#guide':location.pathname);}
