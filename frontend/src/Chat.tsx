...
type Source={forecast?:Record<string,unknown>;data_as_of:string;sql:string;parameters:Record<string,unknown>;metric_versions:Record<string,number>;retrieved_at?:string};
...
export function Chat({access,enabled,advancedEnabled,anchor,language,role}:{access:string;enabled:boolean;advancedEnabled:boolean;anchor:string;language:'vi'|'en';role:string}){
  ...
  function block(a:Answer,title?:string){
    return <>{title&&<h3 className="task-title">{title}</h3>}{message(a)&&<p className={a.status==='denied'?'error':''}>{message(a)}</p>}{a.answer_text&&<p className="answer-summary">{a.answer_text}</p>}<ResultChart config={a.viz_config||null} rows={a.table} language={language}/>{a.table.length>0&&<details className="result-details" open={a.table.length>1}><summary>{vi?'Xem bảng dữ liệu':'View data table'} · {a.table.length}</summary><div className="table-scroll"><table><thead><tr>{Object.keys(a.table[0]).filter(k=>!/id$/i.test(k)).map(k=><th key={k}>{vi?labels[k]||k:k}</th>)}</tr></thead><tbody>{a.table.map((row,i)=><tr key={i}>{Object.entries(row).filter(([k])=>!/id$/i.test(k)).map(([k,v])=><td key={k}>{cell(v,k)}</td>)}</tr>)}</tbody></table></div></details>}{a.sources&&<details className="source-details"><summary>{vi?'Nguồn dữ liệu & cách tính':'Sources & calculation'}</summary><p>{vi?'Ngày tham chiếu':'Data as of'}: {a.sources.data_as_of}</p><p>{vi?'Khoảng truy vấn (ngày kết thúc không bao gồm)':'Query range (end exclusive)'}: {String(a.sources.parameters.start)} — {String(a.sources.parameters.end)}</p><pre>{a.sources.sql}</pre><pre>{JSON.stringify(a.sources.parameters,null,2)}</pre>{a.sources.forecast&&<><p>{vi?'Dự báo (không phải số liệu đo):':'Forecast (not recorded data):'}</p><pre>{JSON.stringify(a.sources.forecast,null,2)}</pre></>}</details>}{a.saved&&<small className="saved-label">✓ {vi?'Đã lưu kết quả':'Result saved'}</small>}</>;
  }
  return <div className="workspace"><aside className="sidebar"><button className="new-chat" onClick={newChat} disabled={busy||recording}><span>＋</span>{vi?'Cuộc trò chuyện mới':'New conversation'}</button><div className="sidebar-title">{vi?'KẾT QUẢ ĐÃ LƯU':'SAVED RESULTS'}<button className="icon-button" onClick={loadHistory} aria-label={vi?'Tải lại lịch sử':'Refresh history'}>↻</button></div>{historyError&&<p className="muted" role="alert">{vi?'Chưa tải được lịch sử.':'History unavailable.'}</p>}<nav className="history-list" aria-label={vi?'Lịch sử':'History'}>{history.map(item=><button key={item.id} className={conversationId===item.conversation_id?'selected':''} disabled={busy||recording} onClick={()=>openResult(item)} title={item.question}>{item.question}</button>)}{!history.length&&<p className="muted">{vi?'Kết quả phân tích của bạn sẽ xuất hiện ở đây.':'Your saved analyses will appear here.'}</p>}</nav><div className="sidebar-foot"><span className="status-dot"/>{vi?'AdventureWorks · Chỉ đọc':'AdventureWorks · Read only'}<small>{role==='production'?'Factory A':role==='sales'?(vi?'Phạm vi: bán hàng':'Scope: sales'):(vi?'Tất cả chỉ số được phê duyệt':'All approved metrics')}</small></div></aside>
    <section className="chat-stage"><div className="chat-toolbar"><strong>{vi?'Trợ lý phân tích':'Analytics assistant'}</strong><span>{vi?'Ngày dữ liệu':'Data as of'} <b>{anchor||'…'}</b></span></div>
      ...
      {turns.map((turn,index)=><div className="turn" key={index}><div className="user-message">{turn.question}</div><div className="assistant-message"><div className="assistant-avatar">A</div><div className="answer-content">{!turn.answer?<p className="thinking">{vi?'Đang phân tích dữ liệu…':'Analyzing data…'}</p>:<>{block(turn.answer,turn.answer.parts?.length?turn.answer.title:undefined)}{(turn.answer.parts||[]).map((part,i)=><section className="task-part" key={i}>{block(part,part.title)}</section>)}</>}</div></div></div>)}<div ref={bottom}/></div>
